...
- コードの修正、機能追加した箇所に関するテストコードを必ず追加する
- 開発作業の終了前に、必ず `cargo check` `cargo build` `cargo test` を実行しユニットテストがパスすること、コード由来のwarningが解消されていることを確認し、問題を修正するまでユーザーに返さない。
- docs/ の修正時には、テストコードの実行は不要。
- 解消できない問題が残る場合、理由を述べること
